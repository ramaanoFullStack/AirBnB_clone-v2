...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Johannesburg"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'Johannesburg'", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="Ramaano" age=29 height=6.2')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Ramaano'", cout.getvalue().strip())
            self.assertIn("'age': 29", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="ramaano.m@gmail.com" password="505"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
            ...
            self.assertIn('ramaano.m@gmail.com', result)
            self.assertIn('505', result)
            cursor.close()
            dbc.close()
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="ramaano.m@gmail.com", password="505")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('ramaano.m@gmail.com', result)
            self.assertIn('505', result)
            self.assertIn('ramaano.m@gmail.com', cout.getvalue())
            self.assertIn('505', cout.getvalue())
            cursor.close()
            dbc.close()
